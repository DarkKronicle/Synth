...
class StatisticConfig(commands.Cog):
    ...
    async def list_config(self, ctx: Context):
        ...
        for channel in config.channels:
            entries.append('<#{0}> {1}'.format(channel, human_allow(config.channels[channel])))
        for user in config.users:
            entries.append('<@{0}> {1}'.format(user, human_allow(config.users[user])))
        for role in config.roles:
            entries.append('<@&{0}> {1}'.format(role, human_allow(config.roles[role])))
    ...
    async def stat_disable(
            self,
            ctx: Context,
            to_disable: typing.Union[discord.TextChannel, discord.VoiceChannel, discord.Member, discord.Role] = None,
    ):
        """
    ...
    async def stat_enable(
            self,
            ctx: Context,
            to_enable: typing.Union[discord.TextChannel, discord.VoiceChannel, discord.Member, discord.Role] = None,
    ):
        """
    ...
    async def stat_remove(
            self,
            ctx: Context,
            to_disable: typing.Union[discord.TextChannel, discord.VoiceChannel, discord.Member, discord.Role],
    ):
        """
